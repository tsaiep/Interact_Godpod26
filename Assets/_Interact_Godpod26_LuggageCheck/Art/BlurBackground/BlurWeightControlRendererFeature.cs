...
public sealed class BlurWeightControlRendererFeature : ScriptableRendererFeature
{
    ...
    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        ...
        BlurWeightControlVolumeComponent myVolume = VolumeManager.instance.stack?.GetComponent<BlurWeightControlVolumeComponent>();
    ...
    private class CustomPostRenderPass : ScriptableRenderPass
    {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            // Initialize the destination with the original color so pixels that fail stencil keep their previous color.
            if (kSampleActiveColor)
            {
                var cameraColorDesc = renderGraph.GetTextureDesc(source);
                ...
                using (var builder = renderGraph.AddRasterRenderPass<CopyPassData>($"{passName} Copy Color", out var passData, profilingSampler))
                {
            ...
            // Sample from the current color texture and only overwrite pixels that pass the material stencil test.
            using (var builder = renderGraph.AddRasterRenderPass<MainPassData>(passName, out var passData, profilingSampler))
            {
                ...
                // Preserve the copied color because stencil testing can skip pixels.
                builder.SetRenderAttachment(destination, 0, kSampleActiveColor ? AccessFlags.ReadWrite : AccessFlags.Write);
